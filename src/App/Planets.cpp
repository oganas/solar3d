...
const float PLANITIA_ORBIT_DISTANCE =
    3.0f * 50.0f;                              // 30.0f units away from Mars
const float PLANITIA_ORBIT_TILT_ANGLE = 15.0f; // Tilt in degrees
...
const float MARS_AU = 31.92f;     // 31.92 * 1.75
...
const float NEPTUNE_AU = 260.08f; // 632.10 * 0.75

const float MOON_ORBIT_DISTANCE = 4.0f * 20.0f;
// PLANITIA_ORBIT_DISTANCE is defined in Planets.h now
...
void handleSolarSystemMotion(float dt) {
  ...
  // --- Planitia Orbit Logic (around Mars) ---
  // Mars is index 4 in the planets array
  ...
  // Calculates a time-varying radius based on the angle and eccentricity
  float eccentricity = PLANITIA_ELLIPTICITY;
  float baseRadius = PLANITIA_ORBIT_DISTANCE;

  // Simple approximation of an elliptical radius (distance varies with angle)
  float orbitalRadius = baseRadius * (1.0f - eccentricity * sin(planitiaAngle));

  // --- TILTED AXIS CALCULATION (Rotation around a tilted axis) ---
  float ORBIT_TILT = glm::radians(PLANITIA_ORBIT_TILT_ANGLE);

  // Position on the orbital plane (X' and Z' axes, where Z' is aligned with Z)
  float planeX = orbitalRadius * cos(planitiaAngle);
  float planeZ = orbitalRadius * sin(planitiaAngle);

  // Tilt the X-coordinate and calculate the vertical Y-offset
  float newX = planeX * cos(ORBIT_TILT);
  float newY = planeX * sin(ORBIT_TILT);

  // Update Planitia's position relative to Mars
  planitia.setPosition(
      glm::vec3(marsPos.x + newX,
                marsPos.y + newY, // The vertical component creates the tilt
                marsPos.z + planeZ));
  // ------------------------------------------
}
